...
class Encoder(nn.Module):
  def __init__(self, in_size, latent_size):  # (612,1200)
    super().__init__()
    self.EA = ExternalAttention(d_model=2772, S=36)
    self.conv1 = nn.Conv1d(in_channels=1, out_channels=2, kernel_size=102, stride=2,padding=50,padding_mode='zeros')
    self.conv2 = nn.Conv1d(in_channels=2, out_channels=3, kernel_size=102, stride=2,padding=50,padding_mode='zeros')
    self.flatten = nn.Flatten()
    self.linear3 = nn.Linear(3*693, latent_size)
  ...
  def forward(self, w):
    try:
        out = self.EA(w)
        out = self.conv1(out)  # (612,2,1386)
        # print(out.shape, '#1')
        out = self.relu(out)
        out = self.conv2(out)  # (612,3，693)
        # print(out.shape, '#2')
        out = self.relu(out)
...
class Decoder(nn.Module):
  def __init__(self, latent_size, out_size):
    super().__init__()
    self.unflatten1 = Unflatten()
    self.conv1 = nn.Conv1d(in_channels=1, out_channels=2, kernel_size=102, stride=2,padding=143,padding_mode='reflect')   # (693)
    self.conv2 = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=3, stride=1,padding=150,padding_mode='reflect')  # (1386)
    self.flatten = nn.Flatten()
    self.linear3 = nn.Linear(1*991, out_size)
    self.relu = nn.ReLU(True)
  ...
  def forward(self, z):
    try :
        out = self.unflatten1(z)  # (612,1,1200)
        # print(z.shape, '#1')
        # print(out.shape, '#2')
        out = self.conv1(out)  # (612,2,693)
        # print(out.shape, '#3')
        out = self.relu(out)
        out = self.conv2(out)  # (612,1,991)
        # print(out.shape, '#4')
        out = self.flatten(out)
