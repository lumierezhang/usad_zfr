...
# normal = pd.DataFrame(np.array(normal)[:6168])
normal = pd.DataFrame(np.array(normal)[:489648])
...
labels = labels[:446808]
# labels = labels[:6168]
attack = attack.drop(["Timestamp", "Normal/Attack" ], axis=1)
attack = pd.DataFrame(np.array(attack)[:446808])
# attack = pd.DataFrame(np.array(attack)[:6168])
...
windows_normal_train = windows_normal[:int(np.floor(.8 *  windows_normal.shape[0]))]  # （395980 24 51）
windows_normal_val = windows_normal[int(np.floor(.8 *  windows_normal.shape[0])):int(np.floor(windows_normal.shape[0]))]  # （98996 24 51）
...
model = UsadModel(w_size,hidden_size)  # 经过三次线性平展以及一次将小于0的数取0进行编码，解码多一次非线性激活函数
...
threshold1 = score[cutoff]
print(threshold1)
#新加
# threshold=0.750 # Decide on your own threshold
